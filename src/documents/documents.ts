...
// Rename a document by id. Unlike get/delete, the WHERE is NOT the authorization: with M5, renaming is
// allowed for write+ team members too, not just the owner, so the route resolves effective access
// (getDocumentAccessForUser + canWriteDocument) BEFORE calling this. A null return means only a race — the
// document was deleted between that check and this update — which the route answers as 404. `updatedAt` is
// bumped because a rename is a modification (it also floats the doc to the top of the dashboard's list).
